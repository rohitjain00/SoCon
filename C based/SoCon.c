 ...
 *             SOCIAL               CONNECTIVITY
 *
 * SoCon stands for  Social Conectivity
 * this project was inspired from the leading tech company facebook
 *
 * In this app one can create n number of accounts and send messages and find his/her friends
 * in order to stay connected as communication is one of the major need in now a days
 *
 * This app is tested in CodeBlocks 16.01 on windows 10 PC
 *
 *      **Running this file on any other PC might not work**
 *              **Due to absence of headers files**
 *
 *
****************************************************************/
...
void dimension(int x,int y) {// to move the cursor to a specific x and y coordinate
    COORD coord;
    coord.X=x;
    coord.Y=y;
...
struct node *head; // all users linked list head
...
void insertNewNode(user *newUser); // defined all the functions here
...
do
{
        ...
        dimension(80,13);
        printf(" 1. Login\n");
        if (loggedin) { // decides weather the user is male or female
            dimension(100,15);
...
void insertNewNode(user *newUser) { //inserting new node in linked list used in registering function
    struct node *temp;
    temp=head;
...
void deleteUser(char userName[40]) // delete any node from the linked list used in delete my account function
{
    if (strcmp(userName,"admin") == 0) { //preserving the admin account
        printf("cannot be deleted..");
        char c[2];
        scanf("%s",c);
        return;
    }
    // else delete all the accounts if user wants so!!...
    ...
    {
        while(strcmp(temp->data->username,userName) != 0) // find the node with the same username
        {
            temp1=temp;
            temp=temp->next;
        }
...
user *searchUser(char userName[40])//returns pointer to the user searched for via username
{
    struct node *temp;
    temp=head;
    if (head==NULL){
    ...
    {
        while(strcmp(temp->data->username,userName) != 0) // parses the linked list till the users is found
        {
            temp=temp->next;
            if (temp==NULL)
            {
...
void addDataTolist() //add all the data from file to the linked list
{
...
void sendMessages()
{
    ...
    char recieverUsername [40];
    dimension(90,12);
          printf("CHIT-CHAT WITH YOUR FRIENDS");
    dimension(90,15);
    printf("To : ");
...
void recieveMessage()
{
    ...
    dimension(90,12);
    printf("INBOX: Check all your messages at one place:)");
    dimension(0,15);
...
void searchSpecificUser()
{
    ...
    soconlogo();
    char userName[40];
    printf("Enter First Name of the user : ");
    scanf("%s",userName) ;
